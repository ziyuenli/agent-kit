...
export function restoreInlineCommentState(
	ctx: ExtensionContext,
): InlineCommentAnnotationSource[] {
	const branch = ctx.sessionManager.getBranch();
	for (let index = branch.length - 1; index >= 0; index--) {
		const entry = branch[index];
		if (entry.type !== "custom" || entry.customType !== INLINE_COMMENT_STATE_TYPE)
			continue;
...
function isInlineCommentAnnotationSource(
	value: unknown,
): value is InlineCommentAnnotationSource {
	return (
		isRecord(value) &&
...
export function findAssistantEntryId(
	...
	for (let index = branch.length - 1; index >= 0; index--) {
		...
		if (
			...
			(renderedMarkdownQuote.length > 0 &&
				markdown.includes(renderedMarkdownQuote))
		) {
			return entry.id;
		}
		...
		if (
			...
			(collapseWhitespace(plain).includes(collapsedQuote) ||
				collapseWhitespace(markdown).includes(collapsedQuote))
		) {
			return entry.id;
		}
	}
