...
For a content edit, inspect the final diff, check English-only prose and changed links, and identify unresolved meaning conflicts. Verify that both native entry points explicitly require reading the shared file, that shared text is not duplicated into them, and that referenced guidance retains its owning scope rather than gaining priority from later reading. A static check does not prove model adherence; report fresh-session behavioral tests separately when performed. For synchronization, additionally require backup and post-write comparison evidence as described in reconciliation.md. Report content, deployment, and Git status separately. Editing a file does not prove discovery by Pi; pulling Git does not prove deployment. Commit, push, or change remote files only within the user's authorization.
