...
export default function inlineComments(pi: ExtensionAPI) {
	...
	function enableInlineComments(
		ctx: ExtensionContext,
		selection?: TranscriptSelection,
	): void {
		if (enabled) return;
		enabled = true;
		const selectionToStage =
			selection ?? getSelectionUI(ctx).getTranscriptSelection?.() ?? lastSelection;
	...
	const openPendingShortcut = async (ctx: ExtensionContext) => {
		...
		if (pendingSelection || selection) {
			...
			if (!pendingSelection && selection && !stageSelection(selection, ctx, true))
				return;
	...
	pi.registerCommand("inline-comments:edit", {
		...
		handler: async (args, ctx) => {
			...
			if (!Number.isInteger(number) || number < 1 || number > comments.length) {
				ctx.ui.notify(
					`Enter a comment number from 1 to ${comments.length || 0}.`,
					"warning",
				);
				return;
			}
